...
can run as a SQL Server Agent job on the **Downtime** instance (<DOWNTIME_SERVER_HOST>)
...
Downtime and CEO_REPORT are both databases on the same instance (<DOWNTIME_SERVER_HOST>).
...
1. **`01_linked_server.sql`** — one-time, run from either database (linked servers are
   ...
   <DOWNTIME_SRC_HOST> (SQL_DOWNTIME_SRC_HOST in .env) (it hosts all 3 plant databases, so one linked server covers all of
